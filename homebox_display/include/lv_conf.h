 ...
 *
 * A real config now, generated from LVGL 9.5.0's own
 * lv_conf_template.h (fetched into
 * .pio/libdeps/esp32-s3/lvgl/lv_conf_template.h by `pio run`) —
 * previously this file was just written-out instructions, which is why
 * the firmware didn't compile. Only 3 things were changed from the
 * template's own defaults, everything else here is template default:
 *   - `#if 0` -> `#if 1` (LVGL ships the template disabled by default)
 *   - LV_USE_NV3007 0 -> 1 (this display's driver; LV_USE_GENERIC_MIPI
 *     turns on automatically as a side effect, see its own #if)
 *   - LV_MEM_SIZE 64K -> 96K (headroom for a few labels + one canvas
 *     object's metadata — NOT the canvas pixel buffer itself, that's
 *     allocated separately from PSRAM in setupDisplay(), see
 *     homebox_display.ino)
 * LV_COLOR_DEPTH (16), LV_USE_CANVAS (1), and LV_USE_LABEL (1) were
 * already the template's own defaults — no change needed there despite
 * earlier instructions here saying to check them.
 */
